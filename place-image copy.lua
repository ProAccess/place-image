...
function Image(img)
    ...
    local docx_cap_par_style = "<w:pPr>" -- Initialize paragraph frame style
    ...
    elseif (FORMAT:match "docx" or FORMAT:match "odt") then -- For WORD DOCX documents
        ...
        elseif frame_position == "center" then
            pos_center = text_width / 2
        else
            pos_center = text_width - (width_in / 2)
        end
        if cap_h_position == "left" then
            cap_docx_ind_l = pos_center - width_in / 2
            cap_docx_ind_r = text_width - cap_docx_ind_l - cap_width_in
        elseif cap_h_position == "center" then
        ...
        docx_cap_par_style = docx_cap_par_style .. '<w:ind w:left="' ..
                                 cap_docx_ind_l * twips_per_in ..
                                 '" w:right = "' .. cap_docx_ind_r *
                                 twips_per_in .. '" />' .. docx_align_x_xml ..
                                 '</w:pPr>'
    ...
    elseif (FORMAT:match "docx" or FORMAT:match "odt") then
        ...
        i, j = string.find(frame_position, "float") -- Is figure floated?
        if i == nil then -- If caption not floated
            v_space_before = '<w:pPr><w:spacing w:before="' .. docx_padding_v ..
                                 '" /></w:pPr>'
            v_space_after = '<w:pPr><w:spacing w:after="' .. docx_padding_v ..
                                '" /></w:pPr>'
            ...
            if (cap_position == "above") then
                if #cap_text > 0 then -- If caption
                    results_cap = v_space_before .. docx_cap_par_style ..
                                      '<w:r>' .. caption_text_docx_style ..
                                      '<w:t>' .. cap_text ..
                                      '</w:t></w:r></w:p><w:p>'
                else
                    results_cap = ""
                ...
                if #cap_text > 0 then -- If caption
                    results_cap = docx_cap_par_style .. '<w:r>' ..
            ...
            if (cap_position == "above") then
                if #cap_text > 0 then -- If caption
                    results_cap =
                        '<w:r>' .. er_msg .. caption_text_docx_style .. '<w:t>' ..
                            cap_text ..
                            '</w:t><w:br w:type="line"/><w:br w:type="line"/></w:r>'
                else
                    results_cap = ""
                end
                results = {
            ...
            elseif (cap_position == "below") then
                if #cap_text > 0 then -- If caption
                    results_cap =
                        '<w:r><w:br w:type="line"/><w:br w:type="line"/>' ..
                            caption_text_docx_style .. '<w:t>' .. er_msg ..
                            cap_text .. '</w:t></w:r>'
                else
                    results_cap = ""
                end
                results = {
                    ...
                    pandoc.RawInline('openxml', results_cap)
                }
            end
        end
        return results
