...
-- function Code(c)
--     print("JUST INTERCEPTED CODE: " .. tostring(c))
--     print("Codeblock attribute: " .. tostring(c.attr))
--     print("Codeblock identifier: " .. tostring(c.identifier))
--     print("Codeblock classes: " .. tostring(cb.classes))
--     if FORMAT:match "docx" then -- For docx documents
--         -- return pandoc.RawInline('openxml',
--         --                         '<w:r><w:rPr><w:rStyle w:val="Codeblock" /></w:rPr><w:t>' ..
--         --                             c.text .. '</w:t></w:r>')
--         return pandoc.RawInline('openxml',
--                                 '<w:r><w:rPr><w:rStyle w:val="Codeblock"/></w:rPr><w:t>' ..
--                                     c.text .. '</w:t></w:r>')
...
--     end
-- end
--                                '<w:r><w:rPr><w:style w:type="character" w:styleId="Codeblock" w:customStyle="true"></w:rPr><w:t>' ..
...
function Image(img)
    ...
    val = getParam("pdf_adjust_lines") -- Latex/PDF imgage wrap height adjust - A text-wrapped Latex/PDF image sometimes will have an extended wrap area at the bottom. In such cases, this provides for an 'adjustment' value to shorten/lengthen wrap area. 
    ...
    elseif FORMAT:match "latex" then -- For Latex/PDF documents
        ...
        if latex_figure_type == "{figure}" then -- if for 'figure'
            ...
            if cap_position == "above" and #cap_text > 0 then
            ...
            results = results .. "\\includegraphics[width=1.0\\textwidth" ..
                          graphic_pos .. "]{" .. src .. "}"
            if cap_position == "below" and #cap_text > 0 then
                results = results .. c_below
            end
    ...
    elseif (FORMAT:match "docx") then
        ...
        if i == nil then -- If caption not floated 
            ...
            if (cap_position == "above") then
                ...
                results = {
                    -- pandoc.RawInline('openxml', results_pre .. results_cap ..
                    --                      results_pre .. img_docx)
                    pandoc.RawInline('openxml',
