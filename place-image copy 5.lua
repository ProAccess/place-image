--[[  -- George Markle 22/09/01
place-image.lua – This filter allows greater control over imgage and caption placement and appearance.
Image may be sized by width, e.g., 35%, 200px, 3cm, 2.5in, etc. 
Captions are optional. Caption options include width, (e.g., 100%, 250px, 1.5in, etc.), font, style (plain, italic, bold, bold-oblique, bold-italic), size (small, normal, large), text-alignment (left, center, right), caption alignment (left, center, right).
Multiple optional figure labels may be defined, e.g., "Figure", "Photo", "My Fantatic Illustration", etc.  Each will be numbered per its label, respectively.
A special parameter, latex_adjust_lines, is provided for those cases where latex misjudges the equivalent line-height of an image.
]] -- Global variables that must be available for both Meta and Image calls
...
local valid_attr_names =
    { -- Names of valid parameters. Each parameter and its value separated by "="
        ...
        "latex_adjust_lines" -- Special parameter for latex/pdf output. Provided for those cases where latex misjudges the equivalent line-height of an image. User may adjust vertical wrap, e.g., 10, 12, 15.
    }
-- Will contain image placement parameters {["param"],{var_default, val_global, val_this}}
local image_params = {}
...
local ltx_cap_r_wd
local ltx_cap_h_pos
local latex_adjust_lines -- 'Adjustment' value to shorten latex/pdf image wrap area.
...
function initialize()
    ...
    image_params["cap_text_size"][default_i] = "small"
...
-- **************************************************************************************************
-- Modify codeblock to show in Word .docx
function CodeBlock(cb)
    print("JUST INTERCEPTED CODE: " .. tostring(cb))
    print("Codeblock attribute: " .. tostring(cb.attr))
    print("Codeblock identifier: " .. tostring(cb.identifier))
    print("Codeblock classes: " .. tostring(cb.classes))
    if FORMAT:match "docs" then -- For html documents
        cb = pandoc.RawInline('openxml',
                              '<w:r><w:rStyle w:val="Code" /><w:t>' .. cb.text ..
                                  '</w:t></w:r>')
        return cb
    else
        return cb
        -- return pandoc.RawInline('openxml', '<w:r><w:rStyle w:val="Code" /><w:t>' ..
        --                             cb.text .. '</w:t></w:r>')
    end
end
...
function Meta(meta)
    ...
    if meta.imageplacement ~= nil then
        ...
        repeat -- Gather any meta-specified global image parameters
            -- print("Evaluating globals" .. glParStr .. "; \nptr: " .. ptr)
            ...
            value = string.match(string.sub(glParStr, j + 1, j + 50),
                                 "[%%%-%+%_%w%.%:%s]+")
            print("Just got VAL: " .. tostring(value))
            if value == nil then
                -- print("EXITING LOOP")
                done = true
                break
            ...
            ptr = j + 1
            -- print("GettingCleanPar")
            err = getCleanPar(key, value, global_i, doctype_overrides) -- Save information
            if #err > 0 then -- If error
                err_msg = err_msg .. err .. "\n"
            end
        until done
        -- print("Completed gathering globals. Any error message is: " .. err_msg)
    else
        print("No 'imageplacement' statement found in Meta section.")
    end
    -- print("Done")
    -- print("Just installed Globals Table: " .. tostring(doctype_overrides))
    -- print("Globals Empty?:" .. tostring(next(doctype_overrides)))
    ...
    -- image_params["width"][global_i] = "50%"
    return meta
end
...
function Image(img)
    ...
    -- print("Processing image attributes: " .. stringify(img.attributes))
    -- parStr = string.gsub(glParStr, "“", '"') -- Clean of any Pandoc open quotes that disables standard expressions
    -- parStr = string.gsub(glParStr, "”", '"') -- Clean of any Pandoc open quotes that disable standard expressions
    -- print("Processing image attributes: " .. tostring(parStr))

    -- print("THIS attributes: " .. tostring(img))
    if #img.attributes ~= 0 then
        ...
        for ptr = 1, #img.attributes, 1 do
            ...
            err = getCleanPar(name, val, this_i, doctype_overrides)
            if #err > 0 then
                --  err_msg = err_msg .. err .. "for figure '" .. src .. "'.\n"
                err_msg = err_msg .. err
            end
        end
        -- print("ATTRIBUTES: " .. r)
    ...
    val = getParam("v_padding") -- Frame vertical padding   
    ...
    val = getParam("h_padding") -- Frame horizontal padding   
    ...
    val = getParam("cap_position") -- Caption position relative to figure 
    -- if verify_entry(val, {"above", "right", "below", "left"}) then
    if verify_entry(val, {"above", "below"}) then
        cap_position = val
    else
        cap_position = image_params["cap_position"][default_i]
        err = err .. "Bad caption position value ('" .. val .. "')" ..
                  for_figure
        print("Detected bad cap_position of " .. val)
    end
    ...
    val = getParam("cap_h_position") -- Caption horizontal position relative to frame
    if verify_entry(val, {"left", "center", "right"}) then
        ...
        err = err .. "Bad caption horizontal position value ('" ..
                  getParam("cap_h_position") .. "')" .. for_figure
    end
    ...
    val = getParam("cap_text_align") -- Caption text align
    if verify_entry(val, {"left", "center", "right"}) then
        cap_text_align = val
        ...
        err = err .. "Bad caption text alignment value ('" .. val .. "')" ..
                  for_figure
    end

    val = getParam("cap_text_style") -- Caption type-face style
    -- if (val ~= nil and #val > 1) then
    if verify_entry(val, text_styles_list) then
        cap_text_style = val -- Get caption style
        -- print("cap_text_ltx_style: " .. cap_text_ltx_style)
        -- print("cap_text_style: " .. cap_text_style ..
        ...
        err = "Bad caption style name ('" .. val .. "')" .. for_figure
    end
    -- end

    val = getParam("cap_text_size") -- Caption text size
    if val ~= nil then
        if verify_entry(val, {"small", "normal", "large"}) then
            ...
            err = err .. "Bad caption size ('" .. val .. "')" .. for_figure
        end
    end
    ...
    val = getParam("cap_label_style") -- Caption figure label style
    if (val ~= nil and #val > 1) then
        if verify_entry(val, text_styles_list) then
            cap_label_style = val
            ...
            err = "Bad caption label style name ('" .. val .. "')" .. for_figure
        end
    end
    ...
    if val ~= nil and val ~= "" then
        -- for i = 1, #val, 1 do
        --     v = string.sub(val, i, i)
        --     print("Separator chars: " .. v .. "-" .. string.byte(v) ..
        --               ", reverse: " .. string.char(string.byte(v)))
        -- end
        -- print("PRE-FILTER: " .. val)
        val = string.gsub(val, "[%“%”%‘%’]", "")
        -- print("POST-FILTER: " .. stringify(val))
        cap_label_sep = val
    else
        cap_label_sep = ""
    end
    ...
    val = getParam("latex_adjust_lines") -- Latex/PDF imgage wrap height adjust - A text-wrapped Latex/PDF image sometimes will have an extended wrap area at the bottom. In such cases, this provides for an 'adjustment' value to shorten/lengthen wrap area. 
    latex_adjust_lines = val
    if latex_adjust_lines ~= nil then
        if type(latex_adjust_lines) ~= integer and
            (tonumber(latex_adjust_lines) < -99 or tonumber(latex_adjust_lines) >
                99) then
            err = err .. "Bad latex height adjustment value ('" ..
                      tostring(latex_adjust_lines) .. "')" .. for_figure ..
                      ". It should be an integer, e.g., '10'.\n"
            latex_adjust_lines = nil
        else
            latex_adjust_lines = tonumber(latex_adjust_lines)
        end
        -- print("Final latex_adjust_lines: " .. tostring(latex_adjust_lines))
    end
    ...
    elseif FORMAT:match "latex" then -- For Latex/PDF documents
        ...
        -- print("latex_adjust_lines: " .. tostring(latex_adjust_lines))
        if latex_adjust_lines ~= nil then -- If wrap lines adjustment specified
            ltx_adjust_lns = "[" .. latex_adjust_lines .. "]" -- Compose code for it
        else
            ltx_adjust_lns = ""
        end
        -- print("PREPARED ltx_adjust_lns: " .. ltx_adjust_lns)
        ltx_positions = {
        ...
        -- print("ltx_position: " .. ltx_position .. "\ncaption_ltx_style; " ..
        --   caption_ltx_style .. "\ncap_text_ltx_style: " ..
        --   cap_text_ltx_style)
        ...
        if #cap_text > 0 then -- If caption
            ...
            -- print("After verifying caption exists, cap_text is: " .. cap_txt)
        ...
        -- print("Combine with style, cap_txt: " .. cap_txt)
        ...
        if latex_figure_type == "{figure}" then -- if for 'figure'
            ...
            if #cap_text > 0 then -- If caption text
                -- print("Placing caption above")
                -- print("padding_v: " .. padding_v .. "; cap_txt: " .. cap_txt)
                if (cap_position == "above") then
    ...
    elseif (FORMAT:match "docx") then
        ...
        -- print("About to label: " .. img_label .. "; Image name is: " ..
        --   tostring(img.label))
...
function getCleanPar(name, value, level, overrides)
    local nam = ""
    local i
    local j
    local doctyp = ""
    ...
    local for_figure
    if src ~= nil then
        for_figure = " specified in markdown file for figure '" .. src ..
                         "'. \n"
    else
        for_figure = " in markdown file Meta 'imageplacement' statement. \n"
    end
    ...
    if i ~= nil then -- If constraint indicated
        ...
        if (doc_specific == true and doctyp == doctype) then
            ...
            -- image_params[nam][doc_specific_i] = attrs[ptr][2] -- Specified so apply if appropriate
            -- print("JUST INSTALLED into table doc-type specific value for " ..
            --   name .. " of " .. value ..
            --   " because this doc type constraint is " .. doctyp ..
            --   " and the doc type is: " .. doctype)
            -- print(
            -- "Confirming installed into table doc-type specific value for " ..
            --     overrides[1][1] .. " of " .. overrides[1][2])
        end
    else -- No doc type constraint
        nam = name
        doc_specific = false
        -- print("No separater: " .. name)
    end

    -- print("Examining: " .. nam .. " with value of " .. value)
    if verify_entry(nam, valid_attr_names) == false then
        err = err .. "Bad attribute name ('" .. name .. "') " .. for_figure
    ...
    if doc_specific == false then
        if #err == 0 then -- Only for valid entries
            ...
            -- print("Just installed value for " .. name .. " of " .. value)
            -- image_params[nam][doc_specific_i] = nil -- Doc type not specified
            -- else
        end
    end
    return err
...
function doctype_override(level, overrides)
    ...
    local nam
    local x
    -- print("Now overriding...")
    -- print("Num: " .. #overrides)
    -- print("Table: " .. tostring(overrides))
    -- print("Empty?:" .. tostring(next(overrides)))
    repeat
        if overrides[ptr] ~= nil then
            nam = overrides[ptr][1]
            image_params[nam][level] = overrides[ptr][2] -- Override non-doc-specific value
            -- print("Overriding: " .. nam .. " at level " .. level .. " with: " ..
            --   overrides[ptr][2])
            -- print("Confirming just wrote " .. image_params[nam][level])
        end
        ptr = ptr + 1
    until ptr > #overrides
end
...
function getParam(p)
    local result
    -- if image_params[p][doc_specific_i] ~= nil then
    --     result = image_params[p][doc_specific_i]
    if image_params[p][this_i] ~= nil then
        result = image_params[p][this_i]
    elseif image_params[p][global_i] ~= nil then
        result = image_params[p][global_i]
    elseif image_params[p][default_i] ~= nil then
        result = image_params[p][default_i]
    else
        result = nil
    end
    return result
