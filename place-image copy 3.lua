...
local text_styles_list = {
    "plain", "italic", "bold", "oblique", "bold-oblique", "bold-italic"
}
    ...
    if meta.imageplacement ~= nil then
        ...
        repeat -- Gather any meta-specified global image parameters
            ...
            if #err > 0 then -- If error
                err_msg = err_msg .. err ..
                              "in Meta global imageplacement statement.  "
            end
        until done
...
function Image(img)
        ...
        if verify_entry(val, text_styles_list) then
            ...
            if (cap_text_style == "bold" or cap_text_style == "bold-oblique" or
                ...
                if cap_text_style == "bold-oblique" or cap_text_style ==
                    "bold-italic" then
                    cap_text_html_style =
                        cap_text_html_style .. "font-style:" .. "italic" .. "; "
                end
            else
    ...
    if (val ~= nil and #val > 1) then
        if verify_entry(val, text_styles_list) then
            ...
            if (cap_label_style == "bold" or cap_label_style == "bold-oblique" or
                ...
                cap_label_html_style = cap_label_html_style .. "font-weight:" ..
                                           "bold" .. "; "
    ...
    elseif FORMAT:match "latex" then -- For Latex/PDF documents
        ...
        if #cap_text > 0 then -- If caption
            cap_txt = caption_ltx_style ..
                          string.gsub(cap_text_ltx_style, "X",
                                      "{" .. cap_lbl .. cap_text .. "}") -- Include any style attributes
        ...
        if i == nil then -- If caption not floated 
            ...
            if (cap_position == "above") then
                if #cap_text > 0 then -- If caption
                    results_cap = docx_cap_par_style .. par_v_space_after ..
                                      '</w:pPr><w:r>' .. er_msg ..
                                      cap_text_docx_style .. '<w:t>' .. cap_lbl ..
...
function getCleanPar(name, value, level, overrides)
    ...
    local for_figure
    if src ~= nil then
        for_figure = " specified for figure '" .. src .. "'\n"
    else
        for_figure = " in Meta imageplacement statement\n"
    end
