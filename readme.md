...
# What’s new in the 2026 filter

The 2026 version preserves the original image/caption placement
capabilities and adds several practical enhancements for SVG workflows,
theming, and metadata handling.

If you’ve already been using this filter, think of this as a
quality-of-life upgrade rather than a reinvention. Existing
image/caption usage should still feel familiar, with a few useful new
knobs to turn.

## New highlights

-   SVG-specific styling directives: `fill`, `stroke_color`, and
    `stroke_width`.
-   New `height` parameter for images, with automatic height derivation
    from width when omitted.
-   Accent theming via YAML (`accent_color` or `accent_palette`) for
    consistent SVG color tokens.
-   More robust `imageplacement` YAML parsing, including key/value
    object format.
-   Inline icon mode via image class `.inline-icon`.
-   Improved debug control via metadata (`debug`, `verbose`),
    environment (`DEBUG`, `VERBOSE`), and Pandoc verbosity.

The net effect: less repetitive hand-tuning, easier color consistency,
and fewer “why is this one image being weird?” episodes.

## Quick examples for the new features

The snippets below are intended to be copy/paste-friendly so you can
test each feature quickly in your own document.

``` markdown
![County map](./images-md/county-map.svg){width=48% fill=#1e3a5f}
```

``` markdown
![Boundary](./images-md/boundary.svg){width=52% fill=accent-light stroke_color=accent-dark stroke_width=1.2}
```

``` markdown
Board vote ![](./images-md/check.svg){.inline-icon width=0.18in fill=accent-dark} passed unanimously.
```
...
accent_palette:
  accent: "#1e3a5f"
  accent-medium: "#2b527f"
  accent-light: "#4f7399"
  accent-lighter: "#7390b0"
  accent-dark: "#17304e"
  accent-darker: "#12263f"
```
...
-   height – Image height. If omitted, the filter may auto-derive it
    from width and intrinsic ratio.
...
-   h_padding, v_padding – Padding between image, caption and
    surrounding text, e.g., `v_padding=15px`
-   fill – (SVG only) Recolors SVG fill. Can be hex (e.g.,
    `fill=#1e3a5f`) or accent token (e.g., `fill=accent-dark`).
-   stroke_color – (SVG only) Recolors SVG stroke. Can be hex or accent
    token.
-   stroke_width – (SVG only) Overrides SVG stroke width.
...
-   cap_position – Vertical positon relative to image: `above` or
    `below`. Default is `above`. `cap_position=above`
-   cap_h\_position – horizontal position of caption block relative to
    image (`left`, `center`, `right`). For example,
    `cap_h_position=right` Default is `center`.
...
-   adjust_frame_ht – (PDF/LaTex documents only) — Used to compensate
    for occasional inaccurate wraps in Pandoc conversions to pdf and
    LaTex formats, e.g., `adjust_frame_ht=8`. It has no effect on other
    formats.
...
In the 2026 filter, you may also use YAML object form, which is often
easier to maintain:
...
This object form is usually easier to read and edit than long
comma-separated strings, especially once you add multiple
...
> Note 1 — If you use legacy one-line `imageplacement` string syntax,
> parameters should be separated by commas. In the 2026 filter, YAML
> object form is also supported and is generally the safer,
...
| height            | Image height. If omitted, filter may derive it from intrinsic aspect ratio when width is known.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | auto    | height=1.4in, height=120px                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| fill              | (SVG only) Recolors SVG fill. Accepts hex values or accent tokens.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | none    | fill=#1e3a5f, fill=accent-dark, fill=accent-light                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| stroke_color      | (SVG only) Recolors SVG stroke. Accepts hex values or accent tokens.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | none    | stroke_color=#17304e, stroke_color=accent-darker                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| stroke_width      | (SVG only) Overrides SVG stroke width.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | none    | stroke_width=1, stroke_width=1.2, stroke_width=2px                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
<figure>
<img src="./images-md/files-organization.png" data-cap_position="above" data-position="center" style="width:75.0%" alt="This illustrates how your files should be organized." /><figcaption aria-hidden="true">This illustrates how your files should be organized.</figcaption>
</figure>
...
<img src="./images-md/happy-face-doc.png" data-pdf.width="35%" data-position="float-right" data-cap_position="above" data-cap_text_align="center" data-adjust_frame_ht="15" style="width:40.0%" alt="Now you can place and caption your images and illustrations wherever and however you like!" />
