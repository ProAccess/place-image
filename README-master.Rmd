---
title:  Place-Image Pandoc Lua Filter Adds Image and Caption Control (2026 Update)
subtitle:  | 
...
The *image-placement.lua* Pandoc filter is intended to address commonly encountered shortcomings when displaying images and captions in documents created by Pandoc from markdown documents. Now you can specify a variety of image and caption parameters directly within markdown images statements. Here are two brief examples. Cut and paste them into your document to try them out, substituting your own images:

![](./images-md/examples.png){width=100%}

# What's new in the 2026 filter

The 2026 version preserves the original image/caption placement capabilities and adds several practical enhancements for SVG workflows, theming, and metadata handling.

If you've already been using this filter, think of this as a quality-of-life upgrade rather than a reinvention. Existing image/caption usage should still feel familiar, with a few useful new knobs to turn.

## New highlights

- SVG-specific styling directives: `fill`, `stroke_color`, and `stroke_width`.
- New `height` parameter for images, with automatic height derivation from width when omitted.
- Accent theming via YAML (`accent_color` or `accent_palette`) for consistent SVG color tokens.
- More robust `imageplacement` YAML parsing, including key/value object format.
- Inline icon mode via image class `.inline-icon`.
- Improved debug control via metadata (`debug`, `verbose`), environment (`DEBUG`, `VERBOSE`), and Pandoc verbosity.

The net effect: less repetitive hand-tuning, easier color consistency, and fewer "why is this one image being weird?" episodes.

## Quick examples for the new features

The snippets below are intended to be copy/paste-friendly so you can test each feature quickly in your own document.
...
- height -- Image height. If omitted, the filter may auto-derive it from width and intrinsic ratio.
- position -- Horizontal position on page (`left`, `center`, `right`, `float-left`, `float-right`). For example, `position=float-right` Floats are text-wrapped.)
- h_padding, v_padding -- Padding between image, caption and surrounding text, e.g., `v_padding=15px`
- fill -- (SVG only) Recolors SVG fill. Can be hex (e.g., `fill=#1e3a5f`) or accent token (e.g., `fill=accent-dark`).
- stroke_color -- (SVG only) Recolors SVG stroke. Can be hex or accent token.
- stroke_width -- (SVG only) Overrides SVG stroke width.
- cap_width -- Width of caption text. If expressed as percent, will be relative to image width, e.g., `cap_width=95%`
- cap_space -- Space between caption and image, e.g., `cap_space=10px`
- cap_position -- Vertical positon relative to image: `above` or `below`. Default is `above`. `cap_position=above`
- cap_h_position -- horizontal position of caption block relative to image (`left`, `center`, `right`). For example, `cap_h_position=right` Default is `center`.
- cap_text_align -- If specified: `left`, `center`, `right`, e.g. `cap_text_align=center`
- cap_text_size -- If specified: `small`, `normal`, `large`. Default is normal. For example, `cap_text_size=small`.
...
- cap_label_sep -- If specified, indicates separator between caption label number and caption. A space character should be represented by an underscore character ("`_`"). For example, for a label like `"Figure 2: My caption"` you would indicate the separator as `":_"`.
- md_cap_ht_adj -- (Markdown/gfm documents only) — Allows tweaking the height of the vertical caption container, should you find a caption failing to be contained properly, e.g., `md_cap_ht_adj=4`.

The following are for PDF/LaTex documents. PDF uses the Latex translator, which has its own algorithms that sometimes produce strange results. The parameters below can be used to *fix* most of these.

- close_frame -- (PDF/LaTex documents only) — Fixes occasional failure by LaTex to close the wrapping-frame below an image, e.g. `close_frame=true`.
- adjust_frame_ht -- (PDF/LaTex documents only) — Used to compensate for occasional inaccurate wraps in Pandoc conversions to pdf and LaTex formats, e.g., `adjust_frame_ht=8`. It has no effect on other formats.
- pdf_anchor_strict -- (PDF/LaTex documents only) — Indicates if PDF/LaTex image may be moved automatically if too close to a top/bottom margin, e.g., `pdf_anchor_strict=true`.

## For headings

- keep_with_next -- Used with *Headings* to keep them from being 'orphaned' at bottom of page, e.g., `keep_with_next=true`. Note, its effect may be compromised if the heading is too close to an image.
...
In the 2026 filter, you may also use YAML object form, which is often easier to maintain:
...
This object form is usually easier to read and edit than long comma-separated strings, especially once you add multiple format-specific overrides.
...
> Note 1 — If you use legacy one-line `imageplacement` string syntax, parameters should be separated by commas. In the 2026 filter, YAML object form is also supported and is generally the safer, easier-to-maintain approach.
...
![This illustrates how your files should be organized.](./images-md/files-organization.png){width=75% width=60% cap_position=above position=center}
...
Natively (without the place-image filter), Pandoc does not create floated images for markdown documents and some *flavors* of markdown further limit image and caption formatting. The place-image filter does allow images to be floated with most markdown *flavors* but with some limitations.
